...
begin
    ...
    status_word <= make_packet(
        ...
        -- 15 enabled, 14..12 log2(window), 11..7 valid samples,
        -- 6..3 write index, 2..0 spare.
            ...
            if reset = '1' then
                ...
                if cmd_valid = '1' and packet_kind(cmd_word) = PKT_KIND_CMD then
                    case packet_code(cmd_word) is
                        when CMD_CONFIG =>
                            -- Payload format:
                            -- 19..16 output destination, 10..8 log2(window):
                            -- 0,1,2,3,4 means 1,2,4,8,16 samples.
                            -- Values above 4 are clamped to 16 samples.
                ...
                if in_valid = '1' and in_ready_i = '1' and packet_kind(in_word) = PKT_KIND_DATA then
                    ...
                    -- Once the buffer is full, subtract the sample that is being
                    -- replaced. Before then, the old sample is logically zero.
                    ...
                    -- The output is valid once the configured window has been
                    -- filled. Arithmetic shift preserves signed values.
...
-- TDMA-MIN facing wrapper for the AVG-ASP core.
entity avg_asp is
    port (
        clock : in  bit_1;
        reset : in  bit_1;
...
begin
    ...
    cmd_valid <= '1' when recv.addr = AVE_ASP_ADDR
                      and packet_kind(recv_word) = PKT_KIND_CMD
                 else '0';

    in_valid <= '1' when recv.addr = AVE_ASP_ADDR
                     and packet_kind(recv_word) = PKT_KIND_DATA
                else '0';

    out_ready <= not status_pending_r;

    u_core : entity work.avg_asp_core
        port map (
            clk         => clock,
            reset       => asp_reset,
            cmd_valid   => cmd_valid,
