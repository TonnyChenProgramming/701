...
ARCHITECTURE sim OF Correlation_ASP_tb IS
    ...
    constant COR_ASP_DEST : std_logic_vector(3 downto 0) := COR_ASP_ADDR(3 downto 0);

    --------------------------------------------------------------------
    -- Cycle counter for latency measurement
    --------------------------------------------------------------------
    signal cycle_count         : natural := 0;
    signal packet_start_cycle  : natural := 0;
    signal timed_packet_active : bit_1 := '0';
...
BEGIN
    ...
    stim_proc : PROCESS
        ...
        ----------------------------------------------------------------
        -- Send one DATA packet and start latency measurement exactly when
        -- DUT samples the full packet at rising_edge(clock).
        ----------------------------------------------------------------
        procedure send_data_timed(
            constant sample_value : natural
        ) is
        begin
            REPORT "Send TIMED DATA sample = " & integer'image(sample_value);

            WAIT UNTIL falling_edge(clock);

            recv.addr <= COR_ASP_ADDR;
            recv.data <= make_data_packet(sample_value);

            WAIT UNTIL rising_edge(clock);

            packet_start_cycle  <= cycle_count;
            timed_packet_active <= '1';

            REPORT "TIMING START: DATA " & integer'image(sample_value) &
                   " sampled at cycle " & integer'image(cycle_count);
        ...
        -- Also reports latency from the timed DATA packet if active.
        ----------------------------------------------------------------
        ...
        begin
            ...
            if timeout = false then
                ...
                if timed_packet_active = '1' then
                    latency_cycles := cycle_count - packet_start_cycle;

                    REPORT msg &
                           " | latency from timed DATA packet = " &
                           integer'image(latency_cycles) &
                           " cycles";
                end if;
